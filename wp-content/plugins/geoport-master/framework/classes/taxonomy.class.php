 ...
 *
 */
class GEOPORTFramework_Taxonomy extends GEOPORTFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'geoport_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && GEOPORT_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = geoport_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $geoport_errors = get_transient( 'geoport-taxonomy-transient' );

    wp_nonce_field( 'geoport-taxonomy', 'geoport-taxonomy-nonce' );

    echo '<div class="geoport-framework geoport-taxonomy geoport-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo geoport_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( geoport_get_var( 'geoport-taxonomy-nonce' ), 'geoport-taxonomy' ) ) {

      $errors = array();
      $taxonomy = geoport_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'geoport_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'geoport_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'geoport_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'geoport_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'geoport_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'geoport-taxonomy-transient', $errors, 10 );
