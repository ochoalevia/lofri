...
Version:      1.0.0
Author:       Ifinger Studio
Author URI:  https://www.ifingerstudio.com/
License: GNU General Public License.
Copyright (C) 2017 Ifinger Studio
 ...
 * @since    0.5
 */
function ifingers_post_like_get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'geoport' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'geoport' );
		$icon = $icon_empty;
	}
	...
	return $output;
} // ifingers_post_like_get_simple_likes_button()
...
function sl_shortcode() {
	return ifingers_post_like_get_simple_likes_button( get_the_ID(), 0 );
} // shortcode()
...
function get_liked_icon() {
	...
	$icon = '<i class="fas fa-heart liked"></i>';
	return $icon;
} // get_liked_icon()
...
function get_unliked_icon() {
	/* If already using Font Awesome with your theme, replace svg with: <i class="fal fa-heart"></i> */
	$icon = '<i class="fal fa-heart unlike"></i>';
	return $icon;
} // get_unliked_icon()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'geoport' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'geoport' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
